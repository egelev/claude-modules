...
export type ClaudeRunResult = { ok: true } | { ok: false; detail: string };
...
async function defaultClaudeRunner(args: string[], env: NodeJS.ProcessEnv): Promise<ClaudeRunResult> {
  try {
    await execFileAsync("claude", args, { env, timeout: INSTALL_TIMEOUT_MS });
    return { ok: true };
  } catch (err) {
 ...
 * Best-effort: every failure path is a logged warning, never a thrown error, since the profile/
